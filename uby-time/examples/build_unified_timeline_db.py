#!/usr/bin/env python3
"""
Build a single cross-scale UBY timeline database from heterogeneous processed datasets.

The output database is intended to demonstrate UBY's core value: using UBY as a
common temporal primary/index key across data sources whose native time systems
...
def _source_doi(source_dataset: str) -> str:
    """Return DOI where a stable DOI is known.

    Many live API/database records do not expose a per-record DOI in the processed
    CSV.  In those cases this is intentionally empty and provenance is retained
    through source_dataset, source_record_uri, and attribution.
    """
...
def _pbdb_events(start_id: int) -> Iterable[UnifiedTimelineEvent]:
    event_id = start_id
    for row in _read_csv(PBDB_CSV):
        source_dataset = _clean(row.get("source_dataset"))
...
def _pbdb_animalia_events(start_id: int) -> Iterable[UnifiedTimelineEvent]:
    event_id = start_id
    for row in _read_csv(PBDB_ANIMALIA_CSV):
        source_dataset = _clean(row.get("source_dataset"))
        min_ma = _clean(row.get("min_ma"))
        max_ma = _clean(row.get("max_ma"))
        accepted_name = _clean(row.get("accepted_name"))
...
def build_events() -> list[UnifiedTimelineEvent]:
    events: list[UnifiedTimelineEvent] = []
    next_id = 1

    for producer in (_ics_events, _pbdb_events, _pbdb_animalia_events, _nasa_events, _usgs_events):
        produced = list(producer(next_id))
        events.extend(produced)
        next_id += len(produced)

    return events


def write_csv(events: list[UnifiedTimelineEvent]) -> None:
    PROCESSED_DIR.mkdir(parents=True, exist_ok=True)
    with CSV_OUT.open("w", encoding="utf-8-sig", newline="") as file:
        writer = csv.DictWriter(file, fieldnames=list(UnifiedTimelineEvent.__dataclass_fields__))
        writer.writeheader()
        for event in events:
            writer.writerow(asdict(event))


def write_sqlite(events: list[UnifiedTimelineEvent]) -> None:
    ...
    with sqlite3.connect(SQLITE_OUT) as conn:
        ...
        conn.executemany(
            f"INSERT INTO uby_events ({', '.join(columns)}) VALUES ({placeholders})",
            [[getattr(event, column) for column in columns] for event in events],
        )
...
def write_metadata(events: list[UnifiedTimelineEvent], wall_seconds: float) -> None:
    ...
    metadata = {
        ...
        "description": (
            "Single-file SQLite database integrating heterogeneous geology, paleontology, "
            "space discovery, and geophysical event datasets through UBY as a common temporal key."
        ),
        ...
        "build_performance": {
            ...
            "records_per_second": len(events) / wall_seconds if wall_seconds else 0,
        },
        "notes": [
...
def main() -> int:
    ...
    write_metadata(events, wall_seconds)
    ...
    print(f"Build records/s: {len(events) / wall_seconds if wall_seconds else 0:.2f}")

    with sqlite3.connect(SQLITE_OUT) as conn:
        print("Category counts:")
        for row in conn.execute("SELECT event_category, COUNT(*) FROM uby_events GROUP BY event_category ORDER BY event_category"):
            print(f"- {row[0]}: {row[1]}")
